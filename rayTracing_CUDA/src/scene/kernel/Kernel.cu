...
__global__ void initCurand(curandStatePhilox4_32_10_t *states,
                           const glm::uvec2 imgDim, const size_t seed) {
...
__device__ glm::vec3
randomDirectionUnitSphere(curandStatePhilox4_32_10_t *rndState) {
  auto rndVec3 = [&rndState]() -> glm::vec3 {
    return 2.f * glm::vec3(curand_uniform(rndState), curand_uniform(rndState),
                           curand_uniform(rndState)) -
           1.f;
  };
...
__device__ HitRecord traceRay(const Ray ray, const Sphere *hittable,
  ...
  for (int i = 0; i < hittableSize; i++) {
    // Shifing current camera to the position of given object. It's used for the
...
__device__ HitRecord closestHit(const Ray ray, float hitDistance,
                                int objectIndex, const Sphere *hittable) {
  const Sphere &closestSphere = hittable[objectIndex];
  glm::vec3 origin =
      ray.origin - closestSphere.getPosition(); // Move back to the origin
  ...
  HitRecord hitRecord(ray.direction, normal, hitPoint, hitDistance,
                      objectIndex);
  return hitRecord;
}
...
__global__ void perPixel(uint32_t *imgBuff, glm::vec3 *accColor,
                         ...
                         const Sphere *hittable, const uint32_t hittableSize,
...
void Kernel::runKernel(const Scene &scene, const Camera &camera,
  ...
  gpuErrChk(
      cudaMalloc(&d_material, scene.material.size() * sizeof(*d_material)));
  gpuErrChk(cudaMalloc(&d_curandState, bufferSize * sizeof(*d_curandState)));
  gpuErrChk(cudaMalloc(&d_camera, sizeof(*d_camera)));
  gpuErrChk(cudaMalloc(&d_settings, sizeof(*d_settings)));
  cudaEventRecord(start);

  auto duration = std::chrono::system_clock::now().time_since_epoch();
  initCurand<<<gridDim, blockDim>>>(d_curandState, imgDim,
  ...
  gpuErrChk(cudaMemcpy(d_material, scene.material.data(),
                       ...
                       cudaMemcpyHostToDevice));
  gpuErrChk(cudaMemcpy(d_camera, &camera, sizeof(*d_camera), 
                      cudaMemcpyHostToDevice));
  gpuErrChk(cudaMemcpy(d_settings, &settings, sizeof(*d_settings),
                      cudaMemcpyHostToDevice));
