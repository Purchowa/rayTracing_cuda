...
static __global__ void trace_ray(uint32_t *d_imgBuff, const glm::uvec2 imgDim,
  ...
  const glm::vec3 rayOrigin = cameraOrigin; // { 0.f, 0.f, 1.f };
  
  glm::vec3 rayDirection = glm::normalize(d_rayDirections[x + y * imgDim.x]);
  const Sphere *closestSphere = nullptr;
  glm::vec3 closestShiftOrigin{};
  float closestT{FLT_MAX};

  for (int i = 0; i < hittableSize; i++) {
    // Shifing current camera to the position of given object. It's for the
  ...
  glm::vec3 normal =
      glm::normalize(closestHit); // normal as unit vector of closestHit
...
void Kernel::runKernel(Scene &scene, Camera camera) {
  // TODO: Jeœli to bêdzie w pêtli siê odœwie¿a³o to warto nie alokowaæ tego za
  // ka¿dym razem
